...
# Kanallarni dinamik saqlash uchun yangi jadval
...
async def sub_check_callback(call: CallbackQuery):
    ...
    if await check_sub(call.from_user.id):
        ...
        if code != "none": 
            await send_anime_direct_videos(call.message, code)
        else: 
            await call.message.answer("✅ Obuna tasdiqlandi! Anime kodini kiriting:")
    else:
        await call.answer("❌ Hali hamma kanallarga obuna bo'lmadingiz!", show_alert=True)

# =========================================================
# 🏗 ADMIN INTEGRATSIYASI VA KANAL BOSHQARUVI
# =========================================================
step = {}
tmp = {}
...
async def add_channel_callback(call: CallbackQuery):
    ...
    await call.message.answer("📝 Qo'shmoqchi bo'lgan kanalingiz usernamesini yuboring (Masalan: @yangi_kanal):")
    await call.answer()
...
async def del_channel_callback(call: CallbackQuery):
    ...
    await call.message.answer("📝 O'chirmoqchi bo'lgan kanalingiz usernamesini aniq yozib yuboring (Masalan: @eski_kanal):")
    await call.answer()

# =========================================================
# 🔄 XABARLARNI QAYTA ISHLASH (STEPS & SEARCH)
# =========================================================
...
async def process_all_steps(m: Message):
    ...
    # KANAL QO'SHISH VA O'CHIRISH JARAYONI
    if step[uid] == "input_add_channel":
        if not m.text.startswith("@"):
            return await m.answer("❌ Kanal usernamesi albatta @ belgisi bilan boshlanishi kerak!")
        sql.execute("INSERT OR IGNORE INTO channels VALUES(?)", (m.text,))
        db.commit(); del step[uid]
        await m.answer(f"✅ {m.text} kanali majburiy obunaga muvaffaqiyatli qo'shildi!", reply_markup=admin_menu)

    elif step[uid] == "input_del_channel":
        sql.execute("DELETE FROM channels WHERE username=?", (m.text,))
        db.commit(); del step[uid]
        await m.answer(f"🗑 {m.text} kanali ro'yxatdan o'chirildi!", reply_markup=admin_menu)
    ...
    # ANIME QO'SHISH JARAYONI
        ...
        tmp[uid]["desc"] = m.text
        step[uid] = "g_video"; await m.answer("🎬 Animening asosiy VIDEOSINI (Treyler yoki 1-qism) yuboring:")
...
async def share_to_channel_callback(call: CallbackQuery):
    if call.from_user.id == ADMIN_ID:
        ...
        # Kanallardan birinchisini post yuborish uchun olamiz
        first_ch = sql.execute("SELECT username FROM channels").fetchone()
        if not first_ch:
            return await call.answer("❌ Majburiy obuna kanallari topilmadi! Biror kanal qo'shing.", show_alert=True)
            
        if data:
            try:
                channel_btn = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="📺 Qismlarni ko'rish (Kanalda)", callback_data=f"channel_watch_{code}")]
                ])
                await bot.send_video(chat_id=first_ch[0], video=data[3], caption=f"🍿 <b>{data[1]}</b>\n\n🆔 Kod: <code>{data[0]}</code>\n\n🎞 {data[2]}", reply_markup=channel_btn)
                await call.answer("✅ Videoli post kanalga yuborildi!", show_alert=True)
...
async def watch_anime_callback(call: CallbackQuery):
    ...
    eps = sql.execute("SELECT * FROM episodes WHERE anime_code=?", (code,)).fetchall()
    if not eps: 
        return await call.answer("❌ Qismlar hali yuklanmagan!", show_alert=True)
