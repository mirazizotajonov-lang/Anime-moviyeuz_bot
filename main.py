...
def admin_menu():
    m = types.ReplyKeyboardMarkup(resize_keyboard=True)
    m.add("➕ Kino qo'shish", "🗑 Kinoni o'chirish")
    m.add("🔙 Asosiy menyuga qaytish")
    return m
...
def qidirish(m):
    if is_sub(m.chat.id):
        msg = bot.send_message(m.chat.id, "Animening kodini kiriting:")
...
@bot.message_handler(func=lambda m: m.text == "🔙 Asosiy menyuga qaytish" and m.from_user.id == ADMIN_ID)
def back_to_main(m):
    bot.send_message(m.chat.id, "Asosiy menyu:", reply_markup=menu())
