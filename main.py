...
# 👑 SENING HAMMA TUGMALARING MAVJUD BO'LGAN ADMIN MENU
...
# =========================================================
# 🛠 ADMIN TUGMALARINI BOSGANDAGI BOSHLANG'ICH AMALLAR
# =========================================================
...
# QOLGAN ADMIN TUGMALARNING STEPLARINI BELGILASH
@dp.message(F.text == "➕ Anime qo'shish")
async def a1(m: Message): 
    if m.from_user.id == ADMIN_ID: step[m.from_user.id] = "a_code"; await m.answer("🎬 Yangi anime uchun KOD kiriting:")

@dp.message(F.text == "➕ Qism qo'shish")
async def a2(m: Message): 
    if m.from_user.id == ADMIN_ID: step[m.from_user.id] = "e_code"; await m.answer("Anime kodini yozing:")
...
# =========================================================
# 🔄 MATNLAR VA JARAYoNLARNI FILTRLASH (XATOSIZ TIZIM)
# =========================================================
@dp.message()
async def main_handler(m: Message):
    uid = m.from_user.id
    
    # AGAR FOYDALANUVChI HECh QANDAY JARAYONDA BO'LMASA - ANIME QIDIRADI
    if uid not in step:
    ...
    # EDITING SYSTEM (NOMINI, RASMINI/VIDEOSINI, TAVSIFINI O'ZGARTIRISH VA O'CHIRISH)
        ...
        tmp[uid] = {"code": m.text}
        step[uid] = "edit_pic_new"; return await m.answer("🎬 Yangi asosiy videoni (Treylerni) yuboring:")
        ...
        db.commit(); del step[uid]; return await m.answer("🗑 Anime va uning barcha qismlari o'chirib tashlandi!", reply_markup=admin_menu)
