...
# ⚙️ ASOSIY SOZLAMALAR
# =========================================================
TOKEN = "8716311558:AAGi2qeuftMnDmcHvyjDgHBDnefFSemwdKU"
ADMIN_ID = 7164685036
...
db = sqlite3.connect("anime_new_clean.db", check_same_thread=False)
sql = db.cursor()
...
async def main_handler(m: Message):
    ...
    if m.text == "🏪 Admin":
        # Bu yerga o'zingning shaxsiy lichkang yoki profiling havolasini qo'yishing mumkin
        btn = InlineKeyboardMarkup(inline_keyboard=[
    ...
    if uid not in step:
        if m.text and not m.text.startswith("/"):
            ...
            if data:
                # Faqat adminga kanalga post yuborish tugmasi ko'rinadi
                if uid == ADMIN_ID:
                    btn = InlineKeyboardMarkup(inline_keyboard=[
